...
Keep public API changes in `src/` paired with focused tests in `tests/`. `src/config/Config.h` and `src/config/Config.cpp` hold the Assignment 1 `DroneConfig`, `MissionConfig`, `LidarConfig`, `MapDimensions`, boundary, resolution, and throwing validation domain models; `DroneConfig::drone_radius` is the Exercise 1 sphere source of truth for clearance and collision checks. `src/io/InputParsers.h` and `src/io/InputParsers.cpp` parse the Assignment 1 fixed input files with documented defaults for missing config keys. Scalar config assignment is table-driven through local `ScalarField` helper tables. Malformed lines, unknown keys, duplicate scalar keys, invalid present values, invalid config validation, and invalid map contents throw `std::runtime_error` so callers can catch and finish `main` normally.
